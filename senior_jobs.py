...
def main():
    ...
    print(f"Filtered: {skipped_exp} not-senior-enough, {skipped_stack} wrong stack, {len(new_jobs)} new.")

    if not new_jobs:
        print("No new senior jobs today.")
        send_telegram("📭 No new matching Senior (3+ yrs) developer jobs today (RapidAPI only test).")
        return

    send_telegram(f"👔 *{len(new_jobs)} Senior (3+ yrs) dev job(s) found today (RapidAPI only test):*")
